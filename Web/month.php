<?php
    ...
    $lastMonth = date("Y-m-d",strtotime("-1 month +1 days"));
?>
...
<body>
	<header class="mainHeader">
		...
		<nav id="menubar">
			<div class="navigate"><a href="index.php#title">aktualne warunki</a></div>
				...
				<script>
					...
					function drawDashboard() {
						temperatures = google.visualization.arrayToDataTable([
          						...
          						<?php
									$result = $db->query("SELECT date, air, dew, heat FROM temperature WHERE date > '" . $lastMonth . "' ORDER BY date;");
				...
				<script>
					...
					function drawDashboard() {
						pressureHumidity = google.visualization.arrayToDataTable([
          						...
          						<?php
									$result = $db->query("SELECT date, pressure, humidity FROM pressure_humidity WHERE date > '" . $lastMonth . "' ORDER BY date;");
									while($row = $result->fetchArray()) {
										$date = str_replace(array(":", " "), "-", $row["date"]);
										list($year, $month, $day, $hour, $minute, $second) = explode("-", $date);
				...
				<script>
					...
					function drawDashboard() {
						air = google.visualization.arrayToDataTable([
          						['data', 'PM25', 'PM10'],
          						<?php
									$result = $db->query("SELECT date, pm25, pm10 FROM air_pollution WHERE date > '" . $lastMonth . "' ORDER BY date;");
									while($row = $result->fetchArray()) {
										$date = str_replace(array(":", " "), "-", $row["date"]);
										list($year, $month, $day, $hour, $minute, $second) = explode("-", $date);
				...
				<script>
					...
					function drawDashboard() {
						wind = google.visualization.arrayToDataTable([
          						...
          						<?php
									$result = $db->query("SELECT date, wind_speed, wind_gust FROM open_weather_API WHERE date > '" . $lastMonth . "' ORDER BY date;");
									while($row = $result->fetchArray()) {
										$date = str_replace(array(":", " "), "-", $row["date"]);
										list($year, $month, $day, $hour, $minute, $second) = explode("-", $date);
				...
				<script>
					...
					function drawDashboard() {
						uv = google.visualization.arrayToDataTable([
          						['data', 'UV'],
          						<?php
									$result = $db->query("SELECT date, uv FROM uv WHERE date > '" . $lastMonth . "' ORDER BY date;");
									while($row = $result->fetchArray()) {
										$date = str_replace(array(":", " "), "-", $row["date"]);
										list($year, $month, $day, $hour, $minute, $second) = explode("-", $date);
				...
				<script>
					...
					function drawDashboard() {
						rain = google.visualization.arrayToDataTable([
          						['data', 'deszcz'],
          						<?php
									$result = $db->query("SELECT date, rain FROM rain WHERE date > '" . $lastMonth . "' ORDER BY date;");
									while($row = $result->fetchArray()) {
										$date = str_replace(array(":", " "), "-", $row["date"]);
										list($year, $month, $day, $hour, $minute, $second) = explode("-", $date);
